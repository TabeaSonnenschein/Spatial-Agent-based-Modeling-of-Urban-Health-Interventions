...
if maketable:
    ...
    for run in poprange:
        experimentdict = IntervRunDict[str(run)]
...
if totTraffVol:
    ...
    AirPollGrid = gpd.sjoin(AirPollGrid, spatial_extract, how="inner", predicate= "intersects")
    print(AirPollGrid.head())
    
        
    AirPollGrid.plot(column="TrV7_8", cmap="viridis", legend=True)
    plt.show()
    
    gridIDsinextract = AirPollGrid["int_id"].unique()

    StatusQuo =  pd.read_csv("D:/PhD EXPANSE/Data/Amsterdam/ABMRessources/ABMData/ModelRuns/StatusQuo/21750Agents/Traffic/TraffViz"+f"/TraffMeans_StatusQuo_MeanAcrossRuns.csv")
    PrkPrice =  pd.read_csv("D:/PhD EXPANSE/Data/Amsterdam/ABMRessources/ABMData/ModelRuns/PrkPriceInterv/21750Agents/Traffic/TraffViz"+f"/TraffMeans_PrkPriceInterv_MeanAcrossRuns.csv")

    StatusQuo = StatusQuo.loc[StatusQuo["int_id"].isin(gridIDsinextract)]
    PrkPrice = PrkPrice.loc[PrkPrice["int_id"].isin(gridIDsinextract)]
    
    # order the dataframes by int_id
    StatusQuo = StatusQuo.sort_values(by="int_id")
    PrkPrice = PrkPrice.sort_values(by="int_id")
    
    analysisdf = pd.DataFrame({"StatusQuo": StatusQuo["mean_prTraff"], "PrkPrice": PrkPrice["mean_prTraff"]})
    analysisdf["Difference"] = analysisdf["PrkPrice"] - analysisdf["StatusQuo"]
    analysisdf["Percentage Change"] = analysisdf["Difference"]/analysisdf["StatusQuo"]
    
    analysisdf.to_csv("D:/PhD EXPANSE/Data/Amsterdam/ABMRessources/ABMData/ParkingPriceTotTraffInterventionAnalysis.csv", index=False)
    print(np.mean(analysisdf["Percentage Change"]))
    
    
    # print(StatusQuo.head())
    # Statmean = np.mean(StatusQuo["mean_prTraff"])
    # Prkmean = np.mean(PrkPrice["mean_prTraff"])
    
    # print(Statmean, Prkmean)
    # difference = Prkmean - Statmean
    # percentagechange = difference/Statmean
    
    # print(difference, percentagechange)
