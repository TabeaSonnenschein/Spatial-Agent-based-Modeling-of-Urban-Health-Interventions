...
if plotvideo:
    while len(unique_activities) < 2:
        ...
        Schedule = scheduledf.loc[scheduledf["SchedID"] == Agentdata["SchedID"].iloc[0]].values
        print(Schedule)
        Schedule= Schedule[0][timestep-2:timestep+7]
        print(Schedule)
        unique_activities = [activity for count, activity in enumerate(Schedule) if activity not in Schedule[:count]]
