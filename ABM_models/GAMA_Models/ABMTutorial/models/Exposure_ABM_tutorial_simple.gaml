...
//	map<string, float> AirPollution_Filter <- create_map(["indoor", "car",  "walk", "bike"], [0.60, 0.80, 1, 1]); /// percentage of Air Pollution that remains (is not filtered)			
	...
	bool drawAirPoll <- true;  
	map<string,rgb> color_per_type <- ["streets"::#aqua, "vegetation":: #green, "buildings":: rgb(40,40,40)];
...
species Noise {
	...
	aspect noise{
		if drawNoise{
			draw shape color: rgb(102, 0, 204, (0.35*(Decibel-40)/15)) border: false;				
		}
	}
}
...
species AirPoll{
	...
	aspect no2_scale {
		if drawAirPoll{
			draw shape color: rgb(102, 0, 204, (0.01* NO2)) border: false;					
		}
	}
}
...
experiment ExposureABM type: gui {
	output {
    	...
    	display stats type: java2D synchronized: true {
			...
			chart "Log Mean Hourly Noise Exposure" type: scatter x_label: (string(int(steps_minute))) + " Minute Steps" y_label: "Log10(Decibel)" background: #black color: #white axes: #white size: {0.5,0.45} position: {0.5, 0.05} label_font: font("SansSerif", 12){
					data "Max hourly exposure" value: log(max(Humans collect each.hourly_mean_Noise)+0.1) color: #red marker: false style: line; 			
					data "Mean hourly exposure" value: log(mean(Humans collect each.hourly_mean_Noise)+0.1) color: #white marker: false style: line;
//					data "Min hourly exposure" value: log(min(Humans collect each.hourly_mean_Noise)) color: #green marker: false style: line;
			}
  		 ...
  		 display map type:opengl {
			...
			graphics GreenSpace refresh: false{
       		...
       		species Noise aspect: noise refresh: false;
			species AirPoll aspect: no2_scale refresh: false;
