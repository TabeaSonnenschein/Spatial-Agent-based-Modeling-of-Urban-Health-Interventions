...
def RetrieveAllTracksHour(agents, dum):
    return [[agent.unique_id, agent.thishourtrack, agent.thishourmode, agent.thishourduration, agent.thishourstarttime, agent.thishourarrival, agent.activities, agent.durationActivties, agent.visitedPlaces]  for agent in agents]
...
class Humans(Agent):
  ...
  - have personal exposure
    ...
    def __init__(self,vector, model):
        ...
        self.thishourtrack, self.thishourmode, self.thishourduration, self.thishourstarttime, self.thishourarrival = [], [], [], [], []
      ...
      if self.current_activity != self.former_activity:
        ...
        if (self.destination_activity != self.pos) and ((self.activity != "traveling") or ((self.activity == "traveling") and (formerdestination != self.destination_activity))):
          if (self.path_memory != 1) or ((self.activity == "traveling") and (formerdestination != self.destination_activity)):
              ...
              self.thishourstarttime.append(current_datetime)
              self.thishourarrival.append(self.arrival_time)
          self.path_memory = 0
          self.newplace = 1
    ...
    def ResetTravelTracks(self, current_datetime):
      if len(self.nexthourstracks) > 0:
        ...
        self.thishourstarttime = [current_datetime]
        self.thishourarrival = [self.arrival_time]
      else:
        self.thishourmode = []
        self.thishourtrack = []
        self.thishourduration = []
        self.thishourstarttime = []
        self.thishourarrival = []
    ...
    def step(self, current_datetime):
        ...
        if self.minute == 0:
            self.ResetTravelTracks(current_datetime)
...
class TransportAirPollutionExposureModel(Model):
    def __init__(self, nb_humans, path_data, crs="epsg:28992",
        ...
        super().__init__(nb_humans, path_data, crs, starting_date, steps_minute)
    ...
    def step(self):
        ...
        if self.minute == 0:
          ...
          Alltracks = pd.DataFrame([item for sublist in Alltracks for item in sublist], columns=['agent', 'geometry', 'mode', 'duration','tripstarttime','triparrival', 'activities', 'activitydurations', 'visitedplaces'])
...
if __name__ == "__main__":  
    ...
    if TraffStage == "Remainder":
      ...
      pd.DataFrame(AirPollGrid[["int_id"]+ traffrestcols]).to_csv(path_data+"ModelRuns/"+modelname+"/" + str(nb_humans)+ f"Agents/Traffic/AirPollGrid_HourlyTraffRemainder_{nb_humans}_pop{subsetnr}_{randomID}.csv", index=False)
