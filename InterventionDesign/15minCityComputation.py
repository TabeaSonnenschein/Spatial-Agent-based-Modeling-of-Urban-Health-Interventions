...
# Restaurants = gpd.read_file("D:/PhD EXPANSE/Data/Amsterdam/Built Environment/Facilities/Amsterdam_Foursquarevenues_Food_RDNew.shp")
# Entertainment = gpd.read_file("D:/PhD EXPANSE/Data/Amsterdam/Built Environment/Facilities/Amsterdam_Foursquarevenues_ArtsEntertainment_RDNew.shp")
# ShopsnServ = gpd.read_file("D:/PhD EXPANSE/Data/Amsterdam/Built Environment/Facilities/Amsterdam_Foursquarevenues_ShopsServ_RDNew.shp")
# Nightlife = gpd.read_file("D:/PhD EXPANSE/Data/Amsterdam/Built Environment/Facilities/Amsterdam_Foursquarevenues_Nightlife_RDNew.shp")
# Professional = gpd.read_file("D:/PhD EXPANSE/Data/Amsterdam/Built Environment/Facilities/Amsterdam_Foursquarevenues_Profess_other_RDNew.shp")
...
print("added", len(newPOIs))
newPOIs.to_file(f"D:/PhD EXPANSE/Data/Amsterdam/Built Environment/Facilities/AgentResidenceIsochrones{walking_time}mMissingGreenSpace.shp", driver="ESRI Shapefile")
POIstats.loc[POIstats["POI"] == "greenspace", "NrNewFacilities"] = len(newPOIs)
POIstats.to_csv(f"D:/PhD EXPANSE/Data/Amsterdam/Built Environment/Facilities/AgentResidenceIsochrones{walking_time}mPOIsStats.csv", index=False)
updatedgreenspace = pd.concat([greenspace, newPOIs])
# dissolve the updated greenspace with the existing greenspace
...
SpatialExtent = gpd.read_feather(path_data+"SpatialData/SpatialExtent.feather")
before = gpd.read_feather(path_data+"SpatialData/Greenspace.feather")
after = gpd.read_file(f"D:/PhD EXPANSE/Data/Amsterdam/Built Environment/Facilities/AgentResidenceIsochrones{walking_time}mUpdatedGreenSpace.shp")
added = gpd.read_file(f"D:/PhD EXPANSE/Data/Amsterdam/Built Environment/Facilities/AgentResidenceIsochrones{walking_time}mMissingGreenSpace.shp")

ax = SpatialExtent.plot( color = None , edgecolor="grey", alpha= 0)
before.plot(ax = ax, color="green")
cx.add_basemap(ax, crs=before.crs, source=cx.providers.CartoDB.PositronNoLabels)
scalebar = ScaleBar(distance_meters, length_fraction=0.2, location="lower right", box_alpha=0.5)  # Adjust the length as needed
ax.add_artist(scalebar)
plt.title(f"Green Spaces before {walking_time}m city scenario")
plt.savefig(f"D:/PhD EXPANSE/Written Paper/04- Case study 1 - Transport Interventions/interventiondocs/15min city/GreenSpace{walking_time}mCityBefore.png", bbox_inches="tight", dpi=600)
plt.close()

ax = SpatialExtent.plot( color = None , edgecolor="grey", alpha= 0)
after.plot(ax = ax, color="green")
added.plot(ax=ax, color="blue")
cx.add_basemap(ax, crs=before.crs, source=cx.providers.CartoDB.PositronNoLabels)
scalebar = ScaleBar(distance_meters, length_fraction=0.2, location="lower right", box_alpha=0.5)  # Adjust the length as needed
ax.add_artist(scalebar)
plt.title(f"Green Spaces after {walking_time}m city scenario")
