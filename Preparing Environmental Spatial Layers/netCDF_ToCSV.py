...
os.chdir(r"C:\Users\Tabea\Documents\PhD EXPANSE\Data\Amsterdam\Weather\since2018")

RH_df = nc.RH.to_dataframe().to_csv('RH.csv')
RH_df = pd.read_csv(os.path.join(os.getcwd(), "RH.csv"))
print(RH_df.columns)
RH_df = RH_df.iloc[list(np.where(RH_df['time'] > "2017-12-30")[0])]
print(RH_df)
RH_df.to_csv('RH.csv', index=False)
stations = RH_df[['station', 'lat', 'lon']].drop_duplicates()
RH_df = RH_df.iloc[list(np.where(RH_df['station'] == 240)[0])]
RH_df.to_csv('Rain_Amsterdam.csv', index=False)


TG_df = nc.TG.to_dataframe().to_csv('TG.csv')
TG_df = pd.read_csv(os.path.join(os.getcwd(), "TG.csv"))
print(TG_df.columns)
TG_df = TG_df.iloc[list(np.where(TG_df['time'] > "2017-12-30")[0])]
print(TG_df)
TG_df.to_csv('TG.csv', index=False)
TG_df = TG_df.iloc[list(np.where(TG_df['station'] == 240)[0])]
TG_df.to_csv('Temperature_Amsterdam.csv',  index = False)
...
FHVEC_df = FHVEC_df.iloc[list(np.where(FHVEC_df['time'] > "2017-12-30")[0])]
print(FHVEC_df)
FHVEC_df.to_csv('FHVEC.csv', index=False)
FHVEC_df = FHVEC_df.iloc[list(np.where(FHVEC_df['station'] == 240)[0])]
FHVEC_df.to_csv('Windspeed_Amsterdam.csv', index=False)
...
DDVEC_df = DDVEC_df.iloc[list(np.where(DDVEC_df['time'] > "2017-12-30")[0])]
print(DDVEC_df)
DDVEC_df.to_csv('DDVEC.csv', index=False)
DDVEC_df = DDVEC_df.iloc[list(np.where(DDVEC_df['station'] == 240)[0])]
DDVEC_df.to_csv('Winddirection_Amsterdam.csv', index=False)
