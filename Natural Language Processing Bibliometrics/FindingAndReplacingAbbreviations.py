...
def FindAbbrev(sentence):
    if len(sentence.strip()) != 0:
        if ("(" and ")" in sentence):
            ...
            if sentence[open_brackets + 1:close_brackets].isupper() and sentence[
                                                                      open_brackets + 1:close_brackets].isalpha() and (
                    len(sentence[open_brackets + 1:close_brackets]) > 1):
