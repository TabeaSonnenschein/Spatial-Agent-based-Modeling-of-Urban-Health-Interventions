...
sum(sexstats$totpop[sexstats$?..age == 15], sexstats$totpop[sexstats$?..age == 25], sexstats$totpop[sexstats$?..age == 45], sexstats$totpop[sexstats$?..age == 65])
...
sexstats$age_group[sexstats$?..age %in% 0:14] = "k_0Tot15Jaar"
sexstats$age_group[sexstats$?..age %in% 15:24] = "k_15Tot25Jaar"
sexstats$age_group[sexstats$?..age %in% 25:44] = "k_25Tot45Jaar"
sexstats$age_group[sexstats$?..age %in% 45:64] = "k_45Tot65Jaar"
sexstats$age_group[sexstats$?..age %in% 65:105] = "k_65JaarOfOuder"
...
for(g in 1:5){   # agegroup indice
  ...
  for(m in min(which(sexstats$age_group == colnam[g])):max(which(sexstats$age_group == colnam[g]))){
    ...
    agents$age[(x[(f+1):(f+nr_people)])] = sexstats$?..age[m]
    f = f + nr_people
  }
}
...
income_data2 = merge(income_data2, income_codes, by.x = "Inkomensklassen",by.y= "�..income_code", all.x = T, all.y = F)
...
c("?..Wijken.en.buurten", "Gemeentenaam" ,"Soort.Regio",   "neighb_code", "Drankgebruik.Voldoet.aan.alcoholrichtlijn" , "Overgewicht.Obesitas",
...
strat_df = merge(strat_df, age_coding, by.x = "LeeftijdReferentiepersoon", by.y= "�..Leeftijd", all.x = T, all.y = F)
